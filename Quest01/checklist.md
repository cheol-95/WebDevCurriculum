...
- MS의 자만으로 소프트웨어 발전이 디뎌졌고 아래의 문제를 발생시켰다.
    - 기능이 적다.
    - 웹 표준을 어긴다. 
    - 지나치게 무겁고 느리다.
    - 버그 수정이 굉장히 느리다.
...
```
ActiveX ?
실행 바이너리를 패키징하여 다른 프로그램에서 쉽게 사용할 수 있도록 객체지향적 인터페이스를 만드는 기술로 아래와 같은 문제점을 발생시켰다.
- 사용자가 의도하지 않은 자동 설치
- 샌드박스 기능 미적용
- 크로스 플랫폼 미지원
- 성능 저하
...
- __header__
    - `<body>`영역의 머리말, 제목 등을 표현해 탐색이 되는 영역
- __selction__
    - `<body>`영역의 본문 콘텐츠를 담고있는 영역
- __article__
    - 블로그 게시물 또는 기술 문서와 같은 재사용 및 신디케이션에 적한합 컨텐츠 영역
- __footer__
    - `<body>`영역의 화면의 구조 중 제일 아래에 위치하며 탐색링크, 맨 위로 돌아가는 링크, 회사소개, 저작권, 약관, 제작정보 등을 담고있는 컨테이너
- __nav__
    - 사이트내에 콘텐츠를 담고 있는 문서들을 서로 연결하는 링크의 집합을 담고있는 영역으로, 주로 메뉴에 사용되며 위치에 영향을 받지 않아 어디에서든 사용 가능
- __aside__
